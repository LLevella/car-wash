...
- Backend этапы 1-24 выполнены.
- Frontend MVP этапы 1-14 выполнены (включая production build/deploy,
  управление автомобилями клиента, обновлённый manager schedule UI,
  автогенерацию TypeScript-типов из OpenAPI, просмотр audit-истории,
  индикатор статуса оплаты и отчёты MVP+).
- Инфраструктурные этапы I1-I9 выполнены.
- Все этапы из текущего плана выполнены. Дальнейшие шаги — оппортунистические
  починки тех-долга (раздел 14) и интеграции (реальный платёжный/SMS-провайдер,
  расширенные отчёты по запросу).
...
| F10    | B18 (компактный schedule payload + детали записи)    | done          |
| F11    | B19 (OpenAPI schema)                                 | done          |
| F12    | B20 (audit log endpoints)                            | done          |
| F13    | B22 (payment status в payload)                       | done          |
| F14    | B24 (manager reports endpoints)                      | done          |
...
- Добавлен endpoint `GET /api/manager/reports/?station=&date_from=&date_to=`
  (`ManagerReportsView`). Параметры обязательны: `date_from` (default
  today), `date_to` (default `date_from`); валидация YYYY-MM-DD и
  `date_to >= date_from`.
- Endpoint защищён `IsManager` и фильтрует по станциям через
  `restrict_queryset_to_accessible_stations`/`can_access_station`,
  как остальные manager-endpoints.
- Aggregates считаются helper-ом `_aggregate_reports`:
  - `bookings_total`, `bookings_by_status` (список `{status, count}`);
  - `revenue_paid` — сумма `paid_amount` по записям с
    `payment_status == "paid"`;
  - `box_utilization`, `washer_utilization` — минуты загрузки по боксам
    и мойщикам, считаются только для активных и `completed` записей.
- URL зарегистрирован под `api:manager:reports`.
- Backend-тесты: агрегация дня (`bookings_total`, `revenue_paid`,
  `box_utilization` 60 мин, `washer_utilization`); валидация диапазона
  возвращает 400 c `validation_error`; rejection при отсутствии доступа
  к станции возвращает 403 c `station_access_denied`.

Критерии готовности:

- manager получает базовые цифры по дню/неделе одной API-вызовом;
- агрегаты покрыты тестами;
- отчётный endpoint защищён правом доступа к станциям и не мешает
  основному booking flow.
...
- Добавлен тип `PaymentStatus` в `api/types.ts` и опциональные поля
  `payment_status`, `paid_amount`, `payment_provider`, `payment_reference`
  в `Booking`.
- Добавлен компонент `PaymentBadge` (`src/components/PaymentBadge.tsx`)
  с локализованными метками («Не оплачено», «Ожидает оплаты»,
  «Оплачено», «Возврат») и semantic-классами для стилей.
- В `CustomerBookingDetailsView` и `ManagerBookingDetailsPage` бейдж
  оплаты выводится рядом с суммой аванса. Запуск/возобновление оплаты
  не добавлен — это будет следующий этап после интеграции с реальным
  провайдером.
- Setup-фикстура расширена payment-полями (`payment_status: "paid"`,
  `paid_amount: "300.00"`, `payment_provider: "stripe"`,
  `payment_reference: "pi_demo_001"`); тест manager booking details
  проверяет рендер бейджа «Оплачено».

Критерии готовности:

- UI показывает статус оплаты как для клиента, так и для менеджера;
- ручной режим без провайдера сохранён: при `payment_status === "unpaid"`
  показывается соответствующая метка, никаких блокирующих CTA нет;
- запуск оплаты — отдельный шаг после интеграции с конкретным провайдером.
...
- Добавлены тип `ManagerReports` (`api/types.ts`) и helper
  `getManagerReports(params)` (`api/manager.ts`).
- Создан `ManagerReportsPage` (`features/manager-reports/`):
  - toolbar c выбором станции и диапазона `С`/`По`;
  - четыре карточки в `reports-grid`: «Сводка» (период, число записей,
    оплачено), «По статусам» (список статусов с локализованными
    метками и счётчиком), «Загрузка боксов» (минуты по боксам),
    «Загрузка мойщиков» (минуты по мойщикам);
  - локальные форматтеры `formatMoney` (RUB locale) и `formatMinutes`
    (`Х ч Y мин`).
- Маршрут `/manager/reports` зарегистрирован под `ProtectedRoute` для
  ролей `manager`/`admin`.
- В `AppShell` добавлен пункт навигации «Отчёты» с иконкой `BarChart3`.
- Графики не добавлены — текстовые карточки достаточны для MVP, и
  библиотека графиков увеличила бы bundle на ~30KB. Графики добавятся
  когда появится конкретный сценарий, требующий визуализации (например,
  тренд на неделю), а не «потому что отчёты обычно с графиками».
- Vitest-тест проверяет, что агрегаты («Завершена», `5 400 ₽`,
  `Бокс #1`, `Мойщик #1`) рендерятся после загрузки данных.

Критерии готовности:

- manager видит дневные/недельные агрегаты в UI без обращения к
  Django admin или БД;
- ошибки запросов и пустые периоды показываются понятным состоянием;
- агрегаты соответствуют backend-контракту (B24) и проверены
  компонентным тестом.
...
Все этапы дорожной карты (B1-B24, F1-F14, I1-I9) выполнены. Дальнейшие
работы — по запросу:

- починки тех-долга из раздела 14: опечатка в `CarDescription`,
  удаление обязательности `Customer.car`, вынос форматтеров и
  `ScheduleGrid`/`ResourceBlockForm` в `components/`, удаление legacy
  `personal/` и `main/`;
- интеграция реального платёжного провайдера поверх payment-ready слоя
  (B22 + F13);
- интеграция SMS/email-провайдера поверх notifications-outbox (B21);
- расширенные отчёты и визуализация (поверх B24 + F14) — графики и
  CSV-экспорт по запросу;
- per-view DRF serializer-ы (вместо `LooseAutoSchema`) для
  type-сильной автогенерации в `types-generated.ts`.
