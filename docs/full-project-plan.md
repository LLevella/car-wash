...
- типы синхронизированы в CI;
- ручные типы удалены.

#### F12. Audit log в UI (после B20)
...
- manager видит историю записи без обращения в админку.
...
`src/api/types.ts` поддерживается параллельно с backend-сериализаторами.
При изменении формы ответа риск рассинхронизации остаётся до перехода на
автогенерацию из OpenAPI.

Решение: этапы B19 (OpenAPI schema) → F11 (генерация типов).
